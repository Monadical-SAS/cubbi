...
def test_suite():
    ...
    # Test 1: Build image
    total_tests += 1
    if run_test("Build Claude Code image", 
    ...
    total_tests += 1
    if run_test("Cubbi image list includes claudecode",
    ...
    session_result = subprocess.run([
        "uv", "run", "-m", "cubbi.cli", "session", "create", 
        "--image", "claudecode", 
    ...
    if session_result.returncode == 0:
        ...
        if session_id:
            ...
            total_tests += 1
            if run_test("Clean up test session",
    ...
    if no_key_result.returncode == 0:
        ...
        if session_id:
            subprocess.run(["uv", "run", "-m", "cubbi.cli", "session", "close", session_id],
    ...
    persist_result = subprocess.run([
        "uv", "run", "-m", "cubbi.cli", "session", "create",
        "--image", "claudecode",
        "--name", "test-persist-auto",
        "--project", "test-automation",
        "--no-connect",
        "--env", "ANTHROPIC_API_KEY=test-key", 
        "--run", "echo 'automation test' > ~/.claude/automation.txt && cat ~/.claude/automation.txt"
    ], capture_output=True, text=True, timeout=60)
    ...
    if tests_passed == total_tests:
        ...
        print(f"❌ {total_tests - tests_passed} test(s) failed. Please check the output above.")
        return False
